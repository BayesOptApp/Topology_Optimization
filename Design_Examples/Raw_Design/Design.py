...
class Design:
    ...
    def __init__(self, 
        ...
        if material_properties_dict is None:
            self._material_properties_dict:dict = {
                'E11':E11_DEFAULT,
                'E22':E22_DEFAULT,
                'G12':G12_DEFAULT,
                'nu12':NU12_DEFAULT,
            ...
            # Check if the material properties are a dictionary
            if isinstance(material_properties_dict, dict):
    ...
    @property
    def material_properties_dict(self)->dict:
        '''
        Returns the material properties dictionary
        '''

        return self._material_properties_dict
    
    @material_properties_dict.setter
    def material_properties_dict(self,new_material_properties_dict:dict)->None:
        '''
        Sets the material properties dictionary

        Inputs:
        - new_material_properties_dict: dict with the material properties
        '''
        if not isinstance(new_material_properties_dict,dict):
            raise ValueError("The new material properties should be a dictionary")
        else:
            partial_dict:dict = DEFAULT_MATERIAL_PROPERTIES.copy()

            # Loop all over the dictionary
            for key in new_material_properties_dict.keys():
                if key in ("E11","E22","G12","nu12"):

                    # Check the values are positive but the nu12
                    if key == "nu12":
                        if new_material_properties_dict[key] < -1.0 or new_material_properties_dict[key] > 0.5:
                            raise ValueError(f"The value of {key} should be between -1 and 0.5")
                    else:
                        if new_material_properties_dict[key] <= 0.0:
                            raise ValueError(f"The value of {key} should be positive")
                    
                    partial_dict[key] = new_material_properties_dict[key]
                else:
                    raise ValueError(f"The key {key} is not a valid material property")
