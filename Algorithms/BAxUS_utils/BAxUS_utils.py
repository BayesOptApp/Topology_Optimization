...
def embedding_matrix(input_dim: int, target_dim: int) -> torch.Tensor:
    r"""Generate a random embedding matrix of size (target_dim, input_dim).
    The embedding matrix is a random binary matrix with values in {-1, 1}."""
...
def increase_embedding_and_observations(
    ...
    r"""
    Increase the number of bins in the embedding matrix S and the number of observations in X.
    The new bins are created by moving the non-zero elements of the rows of S to new bins.
    
    Args
    ------------
    S: torch.Tensor
        The embedding matrix of shape (target_dim, input_dim).  
    X: torch.Tensor
        The observations of shape (n_samples, target_dim).
    n_new_bins: int
        The number of new bins to create in the embedding matrix S.
