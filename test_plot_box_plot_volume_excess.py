...
df_gap = combined_df[["algorithm_name", 
                      "exp_type",  
                      "actual_volume_excess"]].copy()
...
fig = px.box(
    df_gap,
    x="algorithm_name",
    y="actual_volume_excess",
    color="exp_type",
    title="Volume Constraint Distribution by Algorithm and Strategy after full budget",
    labels={
        "actual_volume_excess": "Volume Excess",
...
fig.update_layout(
    ...
    yaxis_title="$V(\\mathbf{x})-V_{\\text{max}}$",
