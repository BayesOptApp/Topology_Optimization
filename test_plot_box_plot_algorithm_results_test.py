...
# Make sure 'data_id' is preserved before converting
#mixed_data_1 = mixed_data_1.with_columns(pl.col("data_id").cast(pl.Int32))  # ensure it’s numeric
#mixed_data_2 = mixed_data_2.with_columns(pl.col("data_id").cast(pl.Int32))
...
# Step 1: Convert dicts to Polars DataFrames
df_class_map = pl.DataFrame({
    "algorithm_name": list(class_map.keys()),
    "algorithm_num": list(class_map.values())
})

df_label_map = pl.DataFrame({
    "algorithm_name": list(algorithm_labels.keys()),
    "algorithm_label": list(algorithm_labels.values())
})

# Step 2: Join with mixed_data_1 and mixed_data_2
mixed_data_1 = mixed_data_1.join(df_class_map, on="algorithm_name", how="left")
mixed_data_2 = mixed_data_2.join(df_class_map, on="algorithm_name", how="left")

mixed_data_1 = mixed_data_1.join(df_label_map, on="algorithm_name", how="left")
mixed_data_2 = mixed_data_2.join(df_label_map, on="algorithm_name", how="left")

# Step 3: Replace algorithm_name with official label (if label is available)
mixed_data_1 = mixed_data_1.with_columns([
    pl.when(pl.col("algorithm_label").is_not_null())
      .then(pl.col("algorithm_label"))
      .otherwise(pl.col("algorithm_name"))
      .alias("algorithm_name")
]).drop("algorithm_label")

mixed_data_2 = mixed_data_2.with_columns([
    pl.when(pl.col("algorithm_label").is_not_null())
      .then(pl.col("algorithm_label"))
      .otherwise(pl.col("algorithm_name"))
      .alias("algorithm_name")
...
mixed_data_1 = mixed_data_1.with_columns([
    pl.lit("Concurrent").alias("exp_type")
])

mixed_data_2 = mixed_data_2.with_columns([
    pl.lit("Sequential").alias("exp_type")
])


# Combine both into one DataFrame
combined_df = pl.concat([mixed_data_1, mixed_data_2], how="align")
...
for alg in algorithm_labels.values():
    # Filter rows for this algorithm only
    print(f"Processing algorithm: {alg}")
    df_alg = combined_df.filter(pl.col("algorithm_name") == alg)
    print(df_alg.select(["data_id", "exp_type", "current_y_best"]).unique().sort("data_id"))

    # Separate by exp_type
    df_concurrent = df_alg.filter(pl.col("exp_type") == "Concurrent").select([
        "data_id", "current_y_best"
    ]).rename({"current_y_best": "Concurrent"})

    df_sequential = df_alg.filter(pl.col("exp_type") == "Sequential").select([
        "data_id", "current_y_best"
    ]).rename({"current_y_best": "Sequential"})
    ...
    concurrent_vals = df_concurrent["Concurrent"].to_numpy()
    sequential_vals = df_sequential["Sequential"].to_numpy()

    # Get the median values
    concurrent_median = np.median(concurrent_vals)
    sequential_median = np.median(sequential_vals)

    # Wilcoxon test
    stat, p = wilcoxon(concurrent_vals, sequential_vals, alternative='two-sided', method="exact")

    wilcoxon_results.append({
        "algorithm": alg,
        "statistic": stat,
        "p_value": p,
        "concurrent_median": concurrent_median,
        "sequential_median": sequential_median
    })

# Print results
print("Wilcoxon Signed-Rank Test: Sequential vs Concurrent")
for res in wilcoxon_results:
    print(f"{res['algorithm']:12s} | Statistic = {res['statistic']:.4f} | Concurrent Median = {res['concurrent_median']:.8f} | Sequential Median = {res['sequential_median']:.8f} | p-value = {res['p_value']:.6f}")
